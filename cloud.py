...
def register_user(username, email, password):
    try:
        ...
        users[username] = {"email": email, "password": hashlib.sha256(password.encode()).hexdigest(), "created": datetime.now().isoformat(), "storage_used": 0, "plan": "free", "settings": {"theme": "light", "notifications": True}, "favorites": []}
                ...
                if len(parts) == 2:
                    ...
                    try:
                        ...
                        if base_name not in files or version > files[base_name]["version"]:
                            files[base_name] = {"filename": base_name, "version": version, "path": file, "size": file.stat().st_size, "modified": datetime.fromtimestamp(file.stat().st_mtime).isoformat(), "icon": get_file_icon(base_name), "is_favorite": base_name in favorites}
                    except:
                        pass
...
def share_file(username, filename, share_with, permission="view"):
    try:
        ...
        log_activity(username, "SHARE", f"Shared {filename}")
        ...
        if auth_mode == "Register":
            ...
            if st.button("🔓 Login", use_container_width=True, type="primary"):
                if authenticate_user(login_user, login_pass):
                    ...
                    if remember_me:
                        save_session(login_user, remember_me=True)
            ...
            if not files:
                ...
                for file in files:
                    with st.container(border=True):
                        c1, c2, c3, c4, c5, c6 = st.columns([4, 0.8, 0.8, 0.8, 0.8, 0.8])
                        with c1:
                        ...
                        if st.session_state.get(f"show_tag_{file['filename']}", False):
                            st.divider()
                            new_tag = st.text_input("Add tag", key=f"nt_{file['filename']}")
